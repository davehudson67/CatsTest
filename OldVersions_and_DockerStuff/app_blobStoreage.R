...

{
  ...
  persistent_container <- storage_container(blob_endp, "persistent-files")
  
  # Define temporary storage container name
  temp_container_name <- "temp-files"
  ...
  if (!(temp_container_name %in% existing_container_names)) {
    create_storage_container(blob_endp, temp_container_name)
    print("Created temporary Azure Blob Storage container: temp-files")
  } else {
    print("Temporary container already exists, skipping creation.")
  }
  
  # Retrieve the container safely
  temp_container <- storage_container(blob_endp, temp_container_name)
  
  loadHtmlFromBlob <- function(blob_container, filename) {
    temp_file <- tempfile()
    storage_download(blob_container, filename, temp_file, overwrite = TRUE)
    paste(readLines(temp_file, warn = FALSE), collapse = "\n")
  }
  
  saveHtmlToBlob <- function(blob_container, content, filename) {
    temp_file <- tempfile()
    writeLines(content, temp_file)
    storage_upload(blob_container, temp_file, filename)
  }
} ## Azure
               ...
               div(class = "page-content-box",
                   fluidRow(
                     ...
                     column(4,
                            ...
                            bsTooltip("useNationalAverages", "Select this option to automatically apply national average Owned Kitten neutering rate of 41%", placement = "top", trigger = "hover"),
                            ...
                            hidden(actionButton("downloadButton", "Download All Runs", class = "neut-button")),
...
server <- function(input, output, session) {
  ...
  {
    # Define the JSON file in Azure Blob Storage
    buttons_file <- "button_config.json"
    
    # Function to load button settings from Blob Storage
    loadButtonsFromBlob <- function(blob_container, filename) {
      temp_file <- tempfile(fileext = ".json")
      
      if (filename %in% list_storage_files(blob_container)$name) {
        storage_download(blob_container, filename, temp_file, overwrite = TRUE)
    ...
    # Function to update only the modified buttons while keeping the rest unchanged
    updateButtonsInBlob <- function(blob_container, updated_data, filename) {
      temp_file <- tempfile(fileext = ".json")
      
      # Load existing JSON data
      existing_data <- loadButtonsFromBlob(blob_container, filename)
      ...
      # Save back to JSON and upload to Blob Storage
      write_json(existing_data, temp_file, auto_unbox = TRUE)
      storage_upload(blob_container, temp_file, filename)
    }
    ...
    # Load button settings from Azure Blob on startup
    observe({
      settings <- loadButtonsFromBlob(persistent_container, buttons_file)
    ...
    # Automatically update the JSON file when the badge checkbox is toggled
    observeEvent(input$showBadge, {
      req(EditPermission())
      
      updated_settings <- list(showBadge = input$showBadge)
      
      updateButtonsInBlob(persistent_container, updated_settings, buttons_file)
    ...
    observeEvent(input$saveButtonSettings, {
      ...
      # Update JSON file in Azure Blob Storage
      updateButtonsInBlob(persistent_container, updated_settings, buttons_file)
    ...
    output$home_buttons <- renderUI({
      ...
      for (i in 1:4) {
        ...
        # Ensure the button settings exist before rendering
        if (!is.null(button_settings[[show_var]]) && isTRUE(button_settings[[show_var]])) {
          buttons <- tagAppendChild(
            buttons,
            tags$a(
              href = ifelse(!is.null(button_settings[[url_var]]), button_settings[[url_var]], "#"), 
              target = "_blank", 
              class = "custom-button", 
              ifelse(!is.null(button_settings[[label_var]]), button_settings[[label_var]], "Unnamed Button")
            )
          )
        }
      }
    ...
    output$dataInfo_Buttons <- renderUI({
      ...
      for (i in 1:4) {
        ...
        # Ensure the button exists before rendering
        if (!is.null(button_settings[[show_var]]) && button_settings[[show_var]]) {
          buttons <- tagAppendChild(
            buttons,
            tags$a(
              href = button_settings[[url_var]], target = "_blank", 
              class = "stacked-button", button_settings[[label_var]]
            )
          )
        }
      }
    ...
    # Render UI inputs for configuring Home Page buttons
    output$home_button_inputs <- renderUI({
      req(button_settings)
      ...
      for (i in 1:4) {
        ...
        inputs <- tagAppendChild(inputs, 
                                 div(
                                   checkboxInput(show_var, paste("Show Button", i), value = button_settings[[show_var]]),
                                   textInput(label_var, paste("Label for Button", i), value = button_settings[[label_var]]),
                                   textInput(url_var, paste("URL for Button", i), value = button_settings[[url_var]])
                                 )
        )
      }
      
      # Add Save Button
      inputs <- tagAppendChild(inputs, actionButton("saveButtonSettings", "Save Home Page Buttons", class = "btn btn-primary"))
      
      inputs
    })
    
    # Render UI inputs for configuring Data Information Tab buttons
    output$data_info_button_inputs <- renderUI({
      req(button_settings)
      ...
      for (i in 1:4) {
        ...
        inputs <- tagAppendChild(inputs, 
                                 div(
                                   checkboxInput(show_var, paste("Show Button", i), value = ifelse(is.null(button_settings[[show_var]]), TRUE, button_settings[[show_var]])),
                                   textInput(label_var, paste("Label for Button", i), value = ifelse(is.null(button_settings[[label_var]]), "", button_settings[[label_var]])),
                                   textInput(url_var, paste("URL for Button", i), value = ifelse(is.null(button_settings[[url_var]]), "", button_settings[[url_var]]))
                                 )
        )
      }
      
      # Add a Save Button
      inputs <- tagAppendChild(inputs, actionButton("saveDataInfoButtons", "Save Data Info Buttons", class = "btn btn-primary"))
  ...
  {
    # Load initial text files from Azure Blob Storage
    homeHtmlPath <- "HomeText.html"
    
    # Reactive value for the large text
    homeSavedHtml <- reactiveVal(loadHtmlFromBlob(persistent_container, homeHtmlPath))
    ...
    observeEvent(input$saveHomeText, {
      if (EditPermission()) {
        ...
        observe({
          ...
          saveHtmlToBlob(persistent_container, input$savedHomeText, homeHtmlPath)
        })
      }
    })
  } ## Home tab  
  ...
  {
    # Define the filename in Azure Blob Storage
    reference_file <- "References.pdf"
    
    output$download_references <- downloadHandler(
      filename = function() {
        "References.pdf"  # Name of the file the user downloads
      },
      content = function(file) {
        temp_file <- tempfile(fileext = ".pdf")
        
        # Download from Azure Blob Storage to a temporary file
        storage_download(persistent_container, reference_file, temp_file, overwrite = TRUE)
        
        # Copy to final location for user download
    ...
    htmlFilePath_top <- "DataInformation_text_top.html"
    htmlFilePath_bottom <- "DataInformation_text_bottom.html"
    
    # Reactive values to hold the saved HTML content for both text blocks
    savedHtml <- reactiveValues(
      text_top = loadHtmlFromBlob(persistent_container, htmlFilePath_top),
      text_bottom = loadHtmlFromBlob(persistent_container, htmlFilePath_bottom)
    )
    ...
    observeEvent(input$saveText_top, {
      if (EditPermission()) {
        ...
        observe({
          ...
          saveHtmlToBlob(persistent_container, input$savedText_top, htmlFilePath_top)
        })
      }
    })
    
    # Save Data Information Text (Bottom)
    observeEvent(input$saveText_bottom, {
      if (EditPermission()) {
        ...
        observe({
          ...
          saveHtmlToBlob(persistent_container, input$savedText_bottom, htmlFilePath_bottom)
        })
      }
    })
    ...
    dataInfo <- reactive({
      tryCatch({
        ...
        storage_download(persistent_container, "DataInfoTable.xlsx", temp_file, overwrite = TRUE)
        temp_data <- read_excel(temp_file)
    ...
    observeEvent(input$dataFile, {
      ...
      tryCatch({
        ...
        storage_upload(persistent_container, input$dataFile$datapath, "DataInfoTable.xlsx")
  ...
  { 
    ...
    # Reactive value for editable top text (Loaded from Azure Blob)
    neuteringSavedHtml <- reactiveVal(loadHtmlFromBlob(persistent_container, neuteringTextPath))
    ...
    observeEvent(input$saveText_neuteringTop, {
      ...
      observe({
        ...
        saveHtmlToBlob(persistent_container, input$savedText_neuteringTop, neuteringTextPath)
        showNotification("Text updated successfully in Azure Blob Storage!", type = "message")
      })
    })
  ...
  ")
    ...
    observeEvent(input$exportRuns, {
      ...
      CatsColourPalete <- c("#7a1b72", "#1a006b", "#511aba", "#7e3be7", "#c928b2", "#e83df5", 
                            "#dec6dc", "#c6bfda", "#d3c6ee", "#dfcef9", "#f1c9ec", "#f9cffc",
                            "#bc8db9", "#8c80b5", "#a88ddd", "#be9df3", "#e493d8", "#f49efa")
      
      # Dynamically assign colors based on the number of unique Runs
      run_names <- unique(comparison_data$Run)
      
      # If there are more runs than colors, recycle the palette
      custom_colors <- setNames(rep(CatsColourPalete, length.out = length(run_names)), run_names)
      ...
      # Pull text from stored file
      homeHtmlPath <- "HomeText.html"
      home_text_content <- loadHtmlFromBlob(persistent_container, homeHtmlPath)
      
      
      # Generate PDF Report
      pdf_report_path <- file.path(export_path, "Population_Report.pdf")
      # Define the dynamic R Markdown content
...
')
      ...
      oldwd <- getwd()
      setwd(export_path)
      zipfile <- file.path(temp_dir, zip_filename)
      zip(zipfile, files = list.files(".", full.names = FALSE))
      setwd(oldwd)
      
      #Upload zip file to Azure Blob Storage (temporary container)
      storage_upload(temp_container, zipfile, zip_filename)
      ...
      output$downloadRuns <- downloadHandler(
        ...
        content = function(file) {
          storage_download(temp_container, zip_filename, file)
        }
      )
    ...
    # Delete temporary blob storage files
    session$onSessionEnded(function() {
      zip_filename <- paste0("exported_runs_", session_id, ".zip")
      if (zip_filename %in% list_storage_files(temp_container)$name) {
        delete_storage_file(temp_container, zip_filename)
        print(paste(" Deleted temporary file:", zip_filename))
      }
    })
  ...
  { 
    mapTextPath <- "MapText.html"  # Filename in Blob Storage
    
    # Reactive value for editable top text (loaded from Azure Blob)
    mapSavedHtml <- reactiveVal(loadHtmlFromBlob(persistent_container, mapTextPath))
    ...
    observeEvent(input$saveText_mapTop, {
      ...
      observe({
        ...
        saveHtmlToBlob(persistent_container, input$savedText_mapTop, mapTextPath)
        showNotification("Text updated successfully in Azure Blob Storage!", type = "message")
      })
    })
    
    # Define reactive value to store the uploaded shapefile
    ...
    observe({
      ...
      shapefile_components <- c(".shp", ".shx", ".dbf", ".prj", ".cpg")
      
      # Download each component separately
      for (ext in shapefile_components) {
        file_to_download <- paste0(shapefile_name, ext)
        temp_file <- file.path(temp_dir, file_to_download)
        storage_download(persistent_container, file_to_download, temp_file, overwrite = TRUE)
      }
      
      # Check if all files exist before loading
      missing_files <- shapefile_components[!file.exists(file.path(temp_dir, paste0(shapefile_name, shapefile_components)))]
      
      if (length(missing_files) > 0) {
        print(paste("Missing shapefile components:", paste(missing_files, collapse = ", ")))
      } else {
        # Load the shapefile and store in reactive value
        shapefile <- st_read(temp_dir, quiet = TRUE)
        uploadedShapefile(shapefile)  # Update reactive value
        print("Successfully loaded shapefile from Azure Blob Storage at startup!")
      }
    })
    ...
    observeEvent(input$loadShapefile, {
      tryCatch({
        ...
        for (ext in shapefile_components) {
          file_to_download <- paste0(shapefile_name, ext)
          temp_file <- file.path(tempDir, file_to_download)
          storage_download(persistent_container, file_to_download, temp_file, overwrite = TRUE)
        }
        
        # Load the shapefile from downloaded components
        newShapefile <- st_read(tempDir, quiet = TRUE)
        uploadedShapefile(newShapefile)  # Store in reactive value
        
        showNotification("Shapefile updated successfully from Azure Blob Storage!", type = "message")
      }, error = function(e) {
        showNotification("Failed to load shapefile. Ensure all required files (.shp, .shx, .dbf, etc.) exist.", type = "error")
      })
    })
    
    # Define temporary directory for storing downloaded shapefile components
    temp_dir <- tempfile()
    dir.create(temp_dir)
    ...
    if (length(missing_files) > 0) {
      print(paste("❌ Missing shapefile components:", paste(missing_files, collapse = ", ")))
      shapefile <- NULL  # Prevents app from crashing
    } else {
      # Load the shapefile
      shapefile <- st_read(temp_dir, quiet = TRUE)
      print("Successfully loaded shapefile from Azure Blob Storage!")
    }
