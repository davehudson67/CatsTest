...
')
  ...
  { 
    ...
    gpkg_path <- "ShapeFiledata/engwales_simplified.gpkg"  # Path to your simplified GPKG
    gpkg_layer <- "engwales"  # Name of the layer inside the GPKG
      ...
      sidebarLayout(
        sidebarPanel(
          selectInput("selected_county", "Select a County:",
                      choices = NULL)  # Populated on server load
        ),
        mainPanel(
          leafletOutput("map", height = "600px"),
          textOutput("clickedAreaInfo")
        )
      )
    )
    
    server <- function(input, output, session) {
      ### --- Reactive value for HTML intro text --- ###
      ...
      observeEvent(input$saveText_mapTop, {
        ...
        observe({
          ...
          saveHtmlToFile(input$savedText_mapTop, mapTextPath)
          showNotification("Text updated successfully!", type = "message")
        })
      })
      
      ### --- Load GeoPackage layer on startup --- ###
      shapefileData <- reactiveVal(NULL)
      
      observe({
        if (file.exists(gpkg_path)) {
          gpkg_data <- st_read(gpkg_path, layer = gpkg_layer, quiet = TRUE)
          shapefileData(gpkg_data)
          updateSelectInput(session, "selected_county",
                            choices = sort(unique(gpkg_data$County_max)))
      ...
      ### --- Filtered data based on selected county --- ###
      selected_data <- reactive({
        req(shapefileData(), input$selected_county)
        shapefileData() %>%
          filter(County_max == input$selected_county)
      })
      ...
      output$map <- renderLeaflet({
        ...
        # Handle NA-only domain
        if (all(is.na(data$Densities_Total))) {
          pal <- colorNumeric("YlOrRd", domain = c(0, 1), na.color = "gray")
        } else {
          pal <- colorNumeric("YlOrRd", domain = data$Densities_Total, na.color = "gray")
        }
      ...
      observeEvent(input$map_shape_click, {
        ...
        if (!is.null(click)) {
          clicked_row <- selected_data() %>% filter(SiteCode == click$id)
          output$clickedAreaInfo <- renderText({
            paste(
              "Selected Area: ", clicked_row$SiteName, "\n",
              "Density: ", ifelse(is.na(clicked_row$Densities_Total), "Unknown", clicked_row$Densities_Total)
            )
          })
        } else {
          output$clickedAreaInfo <- renderText("Click on an area to view details.")
        }
      })
    }
