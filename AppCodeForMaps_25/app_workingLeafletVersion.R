...
server <- function(input, output, session) {
  ...
  ")
    ...
    observeEvent(input$setUp, {
      ...
      if (input$useNationalAverages) {
        rv$TOAUtoOAN <- 0.804565  # variable that is altered to be medium neutering rate
        rv$TStUtoOAN <- as.numeric(gsub("%", "", input$owned_neut_rate_A)) / 100
        rv$TStUtoOJN <- 0.61
      } else {
...
')
  ...
  { 
    ...
    shapefile_dir <- "ShapeFiledata/shapefiles"
