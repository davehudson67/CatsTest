...
# GUI logic
{
  ...
  "))
    ...
    tags$div(
      ...
      img(src = "CP_Logo.png", height = "145px")
    ),
               ...
               div(class = "page-content-box",
                   fluidRow(
                     ...
                     column(4,
                            ...
                            checkboxInput("useNationalAverages", "Use National Average Kitten Neutering Rate", value = FALSE),
                            bsTooltip("useNationalAverages", "Select this option to use the national average kitten neutering rate of 41%", placement = "top", trigger = "hover"),
                            
                            # Uncertainty confidence-interval toggle
                            checkboxInput("showUncertainty", "Show 95% confidence intervals", value = FALSE),
                            bsTooltip("showUncertainty", "Display 95% confidence interval bands (from Monte Carlo simulation) around the predicted populations and growth rates", placement = "top", trigger = "hover"),
                            ...
                            sliderInput("timeframe_pred", "Timeframe of prediction parameters (yrs):",
                                        min = 2, max = 10, value = 10, step = 1, width = "100%", animate = TRUE),
                            ...
                            sliderInput("owned_neut_rate_K", "Owned Kitten Neutering Rate <6months (%):",
                                        min = 5, max = 50, value = 30, step = 5, width = "100%", animate = TRUE),
                            ...
                            actionButton("exportRuns", "Export Run(s) and Plot(s)", class = "neutDownload-button"),
                            downloadButton("downloadRuns", "Download Exported Runs", class = "neutDownload-button", disabled = TRUE),
      ...
      tabPanel(
        ...
        div(class = "page-content-box",
            fluidRow(
              ...
              # Right column for zoomable image
              column(8,
                     tags$div(
          ...
          # Optional editor below if needed
          conditionalPanel(
            condition = "output.showEditor == true",
            h3("Edit Top Text"),
...
server <- function(input, output, session) {
  ...
  # Load the precomputed model cache
  source("model_core.R")
  cache_path <- "data/precomputed_runs.rds"
  precomputed_cache <- if (file.exists(cache_path)) {
    tryCatch(readRDS(cache_path), error = function(e) NULL)
  } else NULL
  
  # Load the precomputed uncertainty cache (Monte Carlo CI bands + growth-rate
  # quantiles for every scenario). Optional: if absent, the app shows point
  # estimates only.
  uncertainty_path <- "data/precomputed_uncertainty.rds"
  uncertainty_cache <- if (file.exists(uncertainty_path)) {
    tryCatch(readRDS(uncertainty_path), error = function(e) NULL)
  } else NULL
  ...
  
  {
    ...
    # Persist the edited home text whenever the editor pushes new content
    observeEvent(input$savedHomeText, {
  ...
  
  {
    ...
    sanitize_data <- function(data) {
      data[] <- lapply(data, function(x) {
        ...
        # replace any NAs with empty string
        x[is.na(x)] <- ""
        x
      })
      data
    ...
    # Persist top text whenever the editor pushes new content
    observeEvent(input$savedText_top, {
    ...
    # Persist bottom text whenever the editor pushes new content
    observeEvent(input$savedText_bottom, {
    ...
    # Reactive value holding the sanitized data to display. Loaded once on start
    # and updated in place on upload, so the render binding is defined only once.
    dataInfo <- reactiveVal({
      tryCatch({
    ...
    # Render table in UI (single binding, reads the reactive value)
    ...
    observeEvent(input$dataFile, {
      ...
      tryCatch({
        ...
        dataInfo(temp_data)  # Update the displayed data; renderTable reacts automatically
      }, error = function(e) {
  ...
  { 
    ...
    # Persist neutering text whenever the editor pushes new content
    observeEvent(input$savedText_neuteringTop, {
  ...
  ")
    ...
    rv <- reactiveValues(Catvec2 = c(2603.18	,876.23	, 8.85	,1597.83	,16.14	,103.09	,1.04	,
                         ...
                         AbsTotal = 0, AbsFeral = 0, AbsOwned = 0, AbsStray = 0, AbsShelter = 0,
                         band = NULL, PG_CI = NULL, zip_path = NULL)
    ...
    resetReactiveValues <- function() {
      ...
      updateCheckboxInput(session, "showUncertainty", value = FALSE)
      updateRadioButtons(session, "owned_neut_rate_A", selected = "95%")
    ...
    observeEvent(input$goButton, {
      ...
      # Try to load from precomputed cache
      nyears <- input$timeframe_pred
      cache_key <- run_key(input$useNationalAverages, input$owned_neut_rate_A, input$owned_neut_rate_K)
      
      run_data <- if (!is.null(precomputed_cache) && !is.null(precomputed_cache$runs[[cache_key]])) {
        # Cache hit: slice the 10-year run to the requested timeframe
        finalize_run(precomputed_cache$runs[[cache_key]], nyears)
      } else {
        # Cache miss or no cache: run live
        showNotification("Running model live (cache miss)", type = "message", duration = 2)
        finalize_run(run_cat_model(
          neut_K = input$owned_neut_rate_K,
          neut_A = input$owned_neut_rate_A,
          useNational = input$useNationalAverages,
          nyears = nyears
        ), nyears)
      }
      
      # Populate reactive values
      rv$Total <- run_data$Total
      rv$Owned <- run_data$Owned
      rv$Feral <- run_data$Feral
      rv$Stray <- run_data$Stray
      rv$Shelter <- run_data$Shelter
      rv$PG_Total <- run_data$PG_Total
      rv$PG_Owned <- run_data$PG_Owned
      rv$PG_Feral <- run_data$PG_Feral
      rv$PG_Stray <- run_data$PG_Stray
      rv$PG_Shelter <- run_data$PG_Shelter
      
      # Uncertainty: look up precomputed Monte Carlo bands for this scenario and
      # slice them to the chosen timeframe. `band[[cat]]` is a (months x 3) matrix
      # of (2.5%, 50%, 97.5%) quantiles; `PG_CI[[cat]]` is the 3-vector of growth-
      # rate quantiles for this timeframe. Only populated when the user enables the
      # "Show 95% confidence intervals" toggle and a cache exists.
      unc <- if (isTRUE(input$showUncertainty) && !is.null(uncertainty_cache)) {
        uncertainty_cache$runs[[cache_key]]
      } else NULL
      if (!is.null(unc)) {
        L <- 12 * nyears
        rv$band <- lapply(unc$band, function(m) m[1:L, , drop = FALSE])
        rv$PG_CI <- lapply(unc$pg, function(p) p[[as.character(nyears)]])
      } else {
        rv$band <- NULL
        rv$PG_CI <- NULL
      }
      ...
      current_run <- list(
        ...
        band = rv$band,        # Monte Carlo CI bands (per-month quantiles)
        PG_CI = rv$PG_CI,      # growth-rate quantiles for this timeframe
        #AbsTotal = rv$AbsTotal,
        #AbsOwned = rv$AbsOwned,
        #AbsFeral = rv$AbsFeral,
        #AbsStray = rv$AbsStray,
        #AbsShelter = rv$AbsShelter,
        parameters = list(
    ...
    # Format a growth rate with its 95% CI when uncertainty is available, e.g.
    # "1.023 (0.981-1.067)"; otherwise just the point estimate. `series` is the
    # category name used to look up rv$PG_CI; `point` is the deterministic value.
    formatGrowth <- function(point, series) {
      pe <- round(median(point), digits = 3)
      ci <- if (!is.null(rv$PG_CI) && !is.null(rv$PG_CI[[series]])) rv$PG_CI[[series]] else NULL
      if (is.null(ci)) {
        as.character(pe)
      } else {
        sprintf("%.3f (%.3f-%.3f)", pe, ci[1], ci[3])
      }
    }
    output$BxOwnedG <- renderText({
      formatGrowth(rv$PG_Owned, "Owned")
    })
    output$BxShelterG <- renderText({
      formatGrowth(rv$PG_Shelter, "Shelter")
    })
    output$BxStrayG <- renderText({
      formatGrowth(rv$PG_Stray, "Stray")
    })
    output$BxFeralG <- renderText({
      formatGrowth(rv$PG_Feral, "Feral")
    })
    
    # Population Plots
    # Shared builder replaces five near-identical renderPlot blocks. `series` is the
    # rv field name (e.g. "Owned"); the per-population differences are just colour,
    # y-axis padding and panel fill.
    buildPopulationPlot <- function(series, line_colour, y_pad, panel_fill) {
      current <- rv[[series]]
      
      # Initial state (no run yet): render an empty plot. An invisible (alpha = 0)
      # single point anchors the axis ranges without drawing anything. A geom_line on
      # one point and an out-of-range annotated point both emit ggplot warnings, so the
      # placeholder is handled here with no line and an x-range that includes the point.
      if (rv$go == 0) {
        start_val <- max(current)
        return(
          ggplot(data.frame(Time = 0, Value = start_val), aes(x = Time, y = Value)) +
            geom_point(size = 4, colour = line_colour, alpha = 0) +
            labs(x = "Time (months)", y = paste(series, "Population")) +
            theme(
            ...
            scale_y_continuous(limits = c(max(0, start_val - y_pad), start_val + y_pad))
        )
      }
      
      if (rv$compare && length(rv$runs) > 1) {
        previous <- rv$runs[[length(rv$runs) - 1]][[series]]
        plot_data <- rbind(
          data.frame(Time = seq_along(previous), Value = previous, Run = "Previous"),
          data.frame(Time = seq_along(current),  Value = current,  Run = "Current")
        )
      } else {
        plot_data <- data.frame(Time = seq_along(current), Value = current, Run = "Current")
      }
      
      # Monte Carlo 95% CI ribbon for the current run, when available. rv$band is a
      # named list of (months x 3) quantile matrices: cols 1/2/3 = 2.5/50/97.5%.
      ribbon_data <- NULL
      if (rv$go != 0 && !is.null(rv$band) && !is.null(rv$band[[series]])) {
        bm <- rv$band[[series]]
        ribbon_data <- data.frame(
          Time  = seq_len(nrow(bm)),
          Lower = bm[, 1],
          Upper = bm[, 3]
        )
      }
      # When comparing, also include the previous run's band so the y-range covers it.
      ribbon_prev <- NULL
      if (rv$go != 0 && rv$compare && length(rv$runs) > 1) {
        pb <- rv$runs[[length(rv$runs) - 1]]$band
        if (!is.null(pb) && !is.null(pb[[series]])) {
          pm <- pb[[series]]
          ribbon_prev <- data.frame(Time = seq_len(nrow(pm)), Lower = pm[, 1], Upper = pm[, 3])
        }
      }
      
      # y-axis range must accommodate the ribbons as well as the lines
      y_vals <- plot_data$Value
      if (!is.null(ribbon_data)) y_vals <- c(y_vals, ribbon_data$Lower, ribbon_data$Upper)
      if (!is.null(ribbon_prev)) y_vals <- c(y_vals, ribbon_prev$Lower, ribbon_prev$Upper)
      
      p <- ggplot(plot_data, aes(x = Time, y = Value))
      if (!is.null(ribbon_prev)) {
        p <- p + geom_ribbon(data = ribbon_prev, inherit.aes = FALSE,
                             aes(x = Time, ymin = Lower, ymax = Upper),
                             fill = line_colour, alpha = 0.10)
      }
      if (!is.null(ribbon_data)) {
        p <- p + geom_ribbon(data = ribbon_data, inherit.aes = FALSE,
                             aes(x = Time, ymin = Lower, ymax = Upper),
                             fill = line_colour, alpha = 0.20)
      }
      p +
        geom_line(data = subset(plot_data, Run == "Previous"),
                  size = 2, colour = line_colour, alpha = 0.3) +
        geom_line(data = subset(plot_data, Run == "Current"),
                  size = 2, colour = line_colour) +
        annotate("point", x = 1, y = max(plot_data$Value[plot_data$Run == "Current"][1]), size = 4, color = line_colour) +
        labs(x = "Time (months)", y = paste(series, "Population")) +
        theme(
          axis.text = element_text(size = 12),
          text = element_text(size = 15),
        ...
        scale_y_continuous(
          limits = c(max(0, min(y_vals) - y_pad), max(y_vals) + y_pad)
        ) +
        theme(legend.position = "none")
    }
    
    # bindCache reuses a rendered plot across re-renders (tab switches, resizes) when
    # the run state and plotted data are unchanged. The data vectors are part of the
    # key so a cached plot can never go stale.
    output$OwnedPlot <- renderPlot({
      buildPopulationPlot("Owned", "#7A1b72", 500, "#f5effb")
    }) %>% bindCache("Owned", rv$go, rv$compare, rv$Owned, rv$band$Owned,
                     if (rv$compare && length(rv$runs) > 1) rv$runs[[length(rv$runs) - 1]]$Owned else NULL)
    output$ShelterPlot <- renderPlot({
      buildPopulationPlot("Shelter", "#C928B2", 50, "#f5effb")
    }) %>% bindCache("Shelter", rv$go, rv$compare, rv$Shelter, rv$band$Shelter,
                     if (rv$compare && length(rv$runs) > 1) rv$runs[[length(rv$runs) - 1]]$Shelter else NULL)
    output$StrayPlot <- renderPlot({
      buildPopulationPlot("Stray", "#7E3BE7", 200, "#f5effb")
    }) %>% bindCache("Stray", rv$go, rv$compare, rv$Stray, rv$band$Stray,
                     if (rv$compare && length(rv$runs) > 1) rv$runs[[length(rv$runs) - 1]]$Stray else NULL)
    output$FeralPlot <- renderPlot({
      buildPopulationPlot("Feral", "#1A006B", 200, "#f5effb")
    }) %>% bindCache("Feral", rv$go, rv$compare, rv$Feral, rv$band$Feral,
                     if (rv$compare && length(rv$runs) > 1) rv$runs[[length(rv$runs) - 1]]$Feral else NULL)
    output$TotalPlot <- renderPlot({
      buildPopulationPlot("Total", "black", 500, "#f5eff5")
    }) %>% bindCache("Total", rv$go, rv$compare, rv$Total, rv$band$Total,
                     if (rv$compare && length(rv$runs) > 1) rv$runs[[length(rv$runs) - 1]]$Total else NULL)
    
    # Generate a unique session ID for file names
    session_id <- paste0("session_", substr(uuid::UUIDgenerate(), 1, 8))
    
    # Define local storage directory. Resolve to an ABSOLUTE path so that every
    # downstream file operation (write/ggsave/render/zip/download) is independent
    # of the current working directory. The export step temporarily setwd()s into
    # this folder; a relative path here would break on the next run if a prior
    # export left the working directory changed ("directory does not exist").
    if (!dir.exists("data/exported_runs")) {
      dir.create("data/exported_runs", recursive = TRUE)
    }
    export_dir <- normalizePath("data/exported_runs", winslash = "/", mustWork = FALSE)
    ...
    observeEvent(input$exportRuns, {
      ...
      rv$zip_path <- file.path(export_dir, zip_filename)
      ...
      combined_data <- do.call(rbind, lapply(seq_along(rv$runs), function(i) {
        run <- rv$runs[[i]]
        if (is.null(run) || length(run$Owned)==0) return(NULL)
        
        owned_count_final   <- round(mean(tail(run$Owned,   12)))
        shelter_count_final <- round(mean(tail(run$Shelter, 12)))
        stray_count_final   <- round(mean(tail(run$Stray,   12)))
        feral_count_final   <- round(mean(tail(run$Feral,   12)))

        owned_pg_final   <- round(median(run$PG_Owned),   3)
        shelter_pg_final <- round(median(run$PG_Shelter), 3)
        stray_pg_final   <- round(median(run$PG_Stray),   3)
        feral_pg_final   <- round(median(run$PG_Feral),   3)
        
        params <- run$parameters
        A_rate <- ifelse(!is.null(params$owned_neut_rate_A), params$owned_neut_rate_A, NA)
        K_rate <- ifelse(!is.null(params$owned_neut_rate_K), params$owned_neut_rate_K, NA)
        
        df <- data.frame(
          Run  = paste0("Run_", i),
          Time = seq_along(run$Owned),
          Owned   = run$Owned,
          Shelter = run$Shelter,
          Stray   = run$Stray,
          Feral   = run$Feral,
          Total   = run$Total,
          owned_neut_rate_A = A_rate,
          owned_neut_rate_K = K_rate
        )
        
        df$Owned_count_final   <- owned_count_final
        df$Shelter_count_final <- shelter_count_final
        df$Stray_count_final   <- stray_count_final
        df$Feral_count_final   <- feral_count_final
        
        df$OwnedPG_final   <- owned_pg_final
        df$ShelterPG_final <- shelter_pg_final
        df$StrayPG_final   <- stray_pg_final
        df$FeralPG_final   <- feral_pg_final
      ...
      lapply(seq_along(rv$runs), function(i) {  
        ...
        plot_data$Population <- factor(plot_data$Population, levels = population_order)
        
        # Add CI ribbon if the run has uncertainty bands
        ribbon_data <- NULL
        if (!is.null(run$band)) {
          bands <- run$band
          # Reshape each band matrix to long format
          ribbon_list <- lapply(names(bands), function(cat) {
            bm <- bands[[cat]]
          ...
          ribbon_data <- do.call(rbind, ribbon_list)
          ribbon_data$Population <- factor(ribbon_data$Population, levels = population_order)
        }
        
        p <- ggplot(plot_data, aes(x = Time, y = Count, color = Population))
        if (!is.null(ribbon_data)) {
          p <- p + geom_ribbon(data = ribbon_data, inherit.aes = FALSE,
                               aes(x = Time, ymin = Lower, ymax = Upper, fill = Population),
                               alpha = 0.2)
        }
        faceted_plot <- p +
          geom_line(size = 1) +
          ...
          scale_color_manual(values = c("Total" = "black", "Owned" = "#7A1b72", "Shelter" = "#C928B2", "Stray" = "#7E3BE7", "Feral" = "#1A006B")) +
          scale_fill_manual(values = c("Total" = "black", "Owned" = "#7A1b72", "Shelter" = "#C928B2", "Stray" = "#7E3BE7", "Feral" = "#1A006B"))
        
        ggsave(filename = file.path(export_dir, paste0("Run_", i, ".png")), plot = faceted_plot, width = 12, height = 8)
      })
      
      # Generate the comparison plot. Reshape with base R and set the factor
      # directly (avoids dplyr mutate, which errors under the installed
      # dplyr/vctrs versions: "vec_is_vector() is defunct").
      comparison_data <- reshape(
        combined_data[, c("Run", "Time", "Total", "Owned", "Shelter", "Stray", "Feral")],
        direction = "long",
        varying   = list(c("Total", "Owned", "Shelter", "Stray", "Feral")),
        v.names   = "Count",
        timevar   = "Population",
        times     = c("Total", "Owned", "Shelter", "Stray", "Feral"),
        idvar     = c("Run", "Time")
      )
      comparison_data$Population <- factor(comparison_data$Population, levels = population_order)
      
      # Add CI ribbons for runs that have uncertainty bands
      ribbon_comp <- NULL
      ribbon_list <- lapply(seq_along(rv$runs), function(i) {
        run <- rv$runs[[i]]
        if (is.null(run) || is.null(run$band)) return(NULL)
        bands <- run$band
        run_name <- paste0("Run_", i)
        lapply(names(bands), function(cat) {
          bm <- bands[[cat]]
          if (is.null(bm) || nrow(bm) == 0) return(NULL)
          data.frame(
            Run = run_name,
      ...
      ribbon_comp <- do.call(rbind, unlist(ribbon_list, recursive = FALSE))
      if (!is.null(ribbon_comp)) {
        ribbon_comp$Population <- factor(ribbon_comp$Population, levels = population_order)
      }
      
      p_comp <- ggplot(comparison_data, aes(x = Time, y = Count, color = Population, group = interaction(Run, Population)))
      if (!is.null(ribbon_comp)) {
        p_comp <- p_comp + geom_ribbon(data = ribbon_comp, inherit.aes = FALSE,
                                        aes(x = Time, ymin = Lower, ymax = Upper, fill = Population, group = interaction(Run, Population)),
                                        alpha = 0.2)
      }
      ...
      # Generate Word Report (PDF output dropped: it required a LaTeX/pdflatex
      # engine that is not installed in this environment).
...
')
      ...
      # Zip files. export_dir and rv$zip_path are absolute, so the archive is created
      # in a working-directory-independent location. We still setwd() into the
      # export folder so the archive stores flat filenames (not nested paths);
      # on.exit() guarantees the working directory is restored even if zip() errors,
      # preventing a stuck wd from breaking subsequent exports.
      oldwd <- getwd()
      on.exit(setwd(oldwd), add = TRUE)
      setwd(export_dir)
      archive_files <- setdiff(list.files(".", full.names = FALSE), zip_filename)
      zip(rv$zip_path, files = archive_files)
      setwd(oldwd)
    ...
    # Download handler for the exported zip file. Defined at the server top level
    # (not inside an observer) so Shiny can register it correctly. The zip path is
    # stored in rv$zip_path by the export observer; if NULL, no export has been run.
    ...
    # Disable download button after it's clicked
  ...
  { 
    ...
    ### --- Load / Save HTML Text Content --- ###
    
    loadHtmlFromFile <- function(filepath) {
      if (file.exists(filepath)) {
        paste(readLines(filepath, warn = FALSE), collapse = "\n")
      } else {
        "<p>Default map text content.</p>"
      }
    }
    ...
    ### --- Reactive Value for Map Text --- ###
    
    mapSavedHtml <- reactiveVal(loadHtmlFromFile(mapTextPath))
    
    # Render the static or editable map text
    ...
    # Persist map text whenever the editor pushes new content
    observeEvent(input$savedText_mapTop, {
