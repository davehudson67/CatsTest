...
server <- function(input, output, session) {
  ...
  ")
    ...
    observeEvent(input$goButton, {
      ...
      # Precompute invariant base population sizes (used for carrying-capacity thresholds)
      ownedBase <- sum(rv$Catvec2[22:28])
      shelterBase <- sum(rv$Catvec2[15:21])
      
      # Matrix precomputation cache: the projection matrix is fully determined by the
      # owned-capacity state, shelter-capacity state and seasonal reproduction factor.
      # There are at most 3 x 3 x 2 = 18 distinct matrices, so we build each only once
      # and reuse it on subsequent iterations instead of rebuilding every step.
      matCache <- list()
      
      # Work on local copies inside the loop to avoid per-iteration reactive writes
      # (each rv$ write copies/invalidates state, and modifying rv$VectorMatrix row by
      # row re-copies the whole matrix every step). Results are written back to rv$
      # once after the loop completes. curMat carries the projection matrix forward,
      # replacing the previously ever-growing rv$MATS list.
      VM <- rv$VectorMatrix
      curMat <- rv$MATS[[1]]
      
      for (i in 1:(rv$lengthoftime - 1)){
        CatsProject <- curMat %*% VM[i, ]
        
        VM[(i+1), ] <- CatsProject
        NOwned <- sum(VM[(i+1), 22:28])
        NShelter <- sum(VM[(i+1), 15:21])
        NStray <- sum(VM[(i+1), 8:14])
        NFeral <- sum(VM[(i+1), 1:7])
        ...
        #matrices are rebuilt (only when this state combination has not been seen before)
        
        matKey <- paste(ownedState, shelterState, rv$SeasonalReproduction[(i+1)], sep = "_")
      ...
      # Write the fully-projected population matrix back to reactive state once
      rv$VectorMatrix <- VM
      ...
      rv$AbsTotal <- mean(rv$Total)
      rv$AbsOwned <- mean(rv$Owned)
      rv$AbsFeral <- mean(rv$Feral)
      rv$AbsStray <- mean(rv$Stray)
      rv$AbsShelter <- mean(rv$Shelter)
