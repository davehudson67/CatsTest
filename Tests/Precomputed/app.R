...
server <- function(input, output, session) {
  ...
  ")
    ...
    buildPopulationPlot <- function(series, line_colour, y_pad, panel_fill) {
      ...
      if (rv$go == 0) {
        plot_data <- data.frame(Time = 0, Value = max(current), Run = "Current")
      } else if (rv$compare && length(rv$runs) > 1) {
      ...
      ggplot(plot_data, aes(x = Time, y = Value)) +
        ...
        scale_y_continuous(
          limits = c(max(0, min(plot_data$Value) - y_pad), max(plot_data$Value) + y_pad)
        ) +
    ...
    }) %>% bindCache("Owned", rv$go, rv$compare, rv$Owned,
    ...
    }) %>% bindCache("Shelter", rv$go, rv$compare, rv$Shelter,
    ...
    }) %>% bindCache("Stray", rv$go, rv$compare, rv$Stray,
